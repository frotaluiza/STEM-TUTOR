...
Sessão com **5 messages**, **26 parts**, agente **general**, modelo **deepseek-v4-flash-free** | 💾 cache read: 85.9K | ⏱️ 0h00m43s

## 👤 Usuário (2026-06-02 01:07:16 UTC)
...
## 🤖 Assistente (2026-06-02 01:07:16 UTC) — Raciocínio
...
## 🤖 Assistente (2026-06-02 01:07:25 UTC) — Raciocínio
...
## 🤖 Assistente (2026-06-02 01:07:31 UTC) — Raciocínio
...
## 🤖 Assistente (2026-06-02 01:07:37 UTC) — Raciocínio
...
3. **ScanRead.ai**
   ...
   - Features: Math→LaTeX, CJK, handwriting, tables
...
## Mathpix Alternatives — Comprehensive Comparison (2026)

| Tool | Open Source | Has API | Pricing | Key Features | Integration Method | Notes |
|---|---|---|---|---|---|---|
| **pix2tex (LaTeX-OCR)** | Yes (MIT) | Yes | Free | ViT encoder + GPT-2 decoder, ~88% exact match, GUI, handwritten support | CLI, Python lib, API server, Docker | 16.4k ★ on GitHub; best open-source starting point |
| **Pix2Text** | Yes (MIT) | Yes | Free | Layout analysis, table OCR, math → LaTeX, 80+ languages, Markdown output | CLI, Python lib, HTTP service | 3.1k ★; full Mathpix alternative pipeline |
...
| **RapidLaTeXOCR** | Yes (MIT) | Yes | Free | ONNX-based port of pix2tex, faster inference, easier deployment | CLI, Python lib (`pip install rapid_latex_ocr`) | 385 ★; good for production deployment |
| **DeepSeek-OCR** | Yes (MIT) | Yes | Free | 3B param VLM, 97% accuracy, formula+table+layout, 100 languages | Transformers, vLLM, REST API | 23.1k ★; SOTA open-source OCR |
...
| **InftyReader** | No | No (desktop app) | £595 one-time | Mature math OCR for scientific docs, Braille output | Desktop GUI | Good for archival, no programmatic use |
...
| **ScanRead.ai** | No | Yes | Free 20 pg/day, Pro $10/mo | Math→LaTeX, CJK, handwriting, 22 specialized tools | REST API | Good free tier for Mathpix-like needs |
...
- **DeepSeekOCR.io** — free online, formula → LaTeX, no signup
- **FreeAIOCR.com** — math image → LaTeX, free + no signup
- **i2OCR.com** — free math equation OCR, premium batch option
- **Mathcheap.xyz** — completely free, OCR + LaTeX editor, no login
- **LaTeX-Tools.online** — OCR + handwriting + table → LaTeX, 100% free
- **YesChat.ai** — image → LaTeX via chat UI

### Top Recommendations

**Best open-source (self-hosted):** Start with **pix2tex/LaTeX-OCR** for pure equation → LaTeX. If you need layout + tables + full document → Markdown, use **Pix2Text**. For highest accuracy, use **TexTeller** (80M training pairs) or **DeepSeek-OCR** (general document understanding with math).
...
*5 messages | 26 parts | 💰 $0.0000 | ⏱️ 0h00m43s*
