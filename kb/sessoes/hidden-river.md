...
Sessão com **8 messages**, **79 parts**, agente **explore**, modelo **deepseek-v4-flash-free** | 💾 cache read: 243.5K | ⏱️ 0h01m14s

## 👤 Usuário (2026-07-11 21:17:52 UTC)
...
## 🤖 Assistente (2026-07-11 21:17:52 UTC) — Raciocínio
...
## 🤖 Assistente (2026-07-11 21:17:57 UTC) — Raciocínio
...
## 🤖 Assistente (2026-07-11 21:18:02 UTC) — Raciocínio
...
## 🤖 Assistente (2026-07-11 21:18:08 UTC) — Raciocínio
...
## 🤖 Assistente (2026-07-11 21:18:22 UTC) — Raciocínio
...
| `factory.py` | **Main entry point** — `complete()` and `stream()` async functions with retry logic, provider resolution, image support |
...
| `provider_factory.py` | `get_runtime_provider()` — builds the correct provider runtime from config |
...
| `traffic_control.py` | `TrafficController` — concurrency semaphore + token-bucket rate limiter |
...
### Multiple Providers Support — **YES, extensively**

The system supports **three access patterns** for multiple providers:

**Pattern A — Provider Registry** (`services/provider_registry.py`):
...
**Pattern B — Provider Core** (`provider_core/`):
Abstract base class `LLMProvider` with concrete implementations:
- `OpenAICompatProvider` — uses `openai.AsyncOpenAI` SDK for all OpenAI-compatible endpoints (OpenAI, DeepSeek, Gemini, Moonshot, gateways, local)
- `AnthropicProvider` — Anthropic SDK for Claude
- `AzureOpenAIProvider` — Azure-specific OpenAI
...
**Pattern C — Direct HTTP** (`cloud_provider.py` / `local_provider.py`):
...
### Layer 1 — `runtime_settings.py` (`RuntimeSettingsService`)
...
- Used by `services/llm/config.py` → `get_llm_config()`

### Layer 4 — Pydantic Settings (`config/settings.py`)
...
### Layer 5 — `config/defaults.py`
- Hardcoded defaults for llm model/provider and paths

### Config Loading Flow:
1. `get_llm_config()` (cached singleton + ContextVar scoped override)
2. → `resolve_llm_runtime_config()` (reads `model_catalog.json` via `ModelCatalogService`)
3. → `ModelCatalogService.load()` (reads/creates/normalizes JSON)
4. → Provider auto-detection via `provider_registry.py`
5. → Returns `LLMConfig` dataclass with all resolved fields
...
### User Roles
- `admin` — full access to admin workspace (`data/user/`)
- `user` — restricted to granted resources from `data/users/<uid>/`
...
### Rate Limiting — `traffic_control.py`
...
*8 messages | 79 parts | 💰 $0.0000 | ⏱️ 0h01m14s*
